# Pam250 chart for matched amino acids
Pam250 = {'A': {'A': 2, 'C': -2, 'E': 0, 'D': 0, 'G': 1, 'F': -3, 'I': -1, 
...
def FindMaxScore(A, B, delta):
    ...
    L = create_matrix(len(A) + 1, len(B) + 1)
    ...
    for i in range(1, len(A)+1):
        for j in range(1, len(B)+1):
            ...
            BDel = F[i-1][j] + delta
            #Diagonal Score
            Diagonal = F[i-1][j-1] + match_score(A[i-1], B[j-1])
            # The locl alignment recurrence rule;
            Maxscore = max(ADel, BDel, Diagonal, 0)
            F[i][j] = Maxscore
...
def LocalAlignment(best, FinalA, FinalB):
    print (best)
    print(FinalA)
    print(FinalB)
...
B = lines[1]
#print (A[5], B[6])
#print (match_score(A[5], B[6]))
L, best, oploc = FindMaxScore(A, B, delta)
FinalA, FinalB = calAlignment(A, B, L, oploc)
LocalAlignment(best, FinalA, FinalB)